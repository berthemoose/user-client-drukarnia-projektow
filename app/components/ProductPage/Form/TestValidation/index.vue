<template>
  <div class="border-2 border-dashed border-gray-400 p-4 rounded">
    <p class="font-bold mb-2">🧪 Server Validation Test</p>
    ...
    <div v-if="result" class="mt-4 p-3 rounded text-sm" :class="result.valid ? 'bg-green-100' : 'bg-red-100'">
  ...
  try {
    const response = await $fetch('/api/orders/validate-price', {
      ...
      body: {
        ...
        pages: props.pages,
        copies: props.copies,
        clientCalculatedPrice: props.clientCalculatedPrice
      }
    });
