<template>
  ...
  <div
    v-if="!pending && !error && topSection?.mainImage"
  ...
  >
    ...
    <div class="relative h-[25vh] md:h-[30vh] lg:h-[35vh] xl:h-[40vh] w-full">
      <!-- Background image -->
      <img
        :src="topSection.mainImage.url"
        :alt="topSection.mainImage.alt || 'Tło sekcji o nas'"
      ...
      >
        <div class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-4 lg:pb-6">
          <h1
            class="text-2xl font-bold text-white sm:text-3xl md:text-4xl lg:text-5xl max-w-3xl"
          >
            {{ topSection?.header || "O Naszej Firmie" }}
          </h1>
          <p
            v-if="topSection?.paragraph"
            class="mt-1 text-white/90 text-base sm:text-lg max-w-2xl"
          >
  ...
  <div v-if="pending" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-50">
    <Loader :text="'Wczytywanie strony: O nas'" :isFullScreen="true" />
  </div>

  <div v-else-if="error" class="bg-gray-50 pt-12 px-4 sm:px-6 lg:px-8">
    <!-- Error State -->
  ...
  <div v-else class="bg-gray-50 pt-12 px-4 sm:px-6 lg:px-8">
      <!-- Content -->
      <div class="max-w-7xl mx-auto">
        <!-- Timeline Section -->
        <div
          v-if="transformedTimelineItems.length > 0"
        ...
        <div 
          v-if="ctaSection" 
          class="bg-white py-16 px-4 sm:px-6 lg:px-8 transition-all duration-700 ease-out delay-200"
          :class="isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'"
        >
          <div class="max-w-3xl mx-auto text-center">
            ...
            <p
              v-if="ctaSection.paragraph"
              class="mt-1 text-base text-gray-600 max-w-xl mx-auto text-xl"
            >
            ...
            <div v-if="hasValidButton(ctaSection.primaryButton) || hasValidButton(ctaSection.secondaryButton)" class="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
              <NuxtLink
                v-if="hasValidButton(ctaSection.primaryButton)"
                :to="ctaSection.primaryButton.url"
              ...
              <NuxtLink
                v-if="hasValidButton(ctaSection.secondaryButton)"
                :to="ctaSection.secondaryButton.url"
            ...
            <p v-if="ctaSection.phoneNumber" class="mt-4 text-sm sm:text-base text-gray-500">
              Lub zadzwoń:
              <a
                :href="`tel:${ctaSection.phoneNumber.replace(/\s+/g, '')}`"
